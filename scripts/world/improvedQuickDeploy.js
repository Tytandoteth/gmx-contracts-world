...
async function main() {
  ...
  if (!deploymentData.WLD) {
    ...
    const WLD = await ethers.getContractFactory("BaseToken");
    ...
    const mintTx = await wldToken._mint(deployer.address, expandDecimals(1000000, 18));
    await mintTx.wait();
  ...
  if (!deploymentData.WWORLD) {
    ...
    const WWORLD = await ethers.getContractFactory("BaseToken");
    ...
    // Mint some tokens for testing
    const mintTx = await wworldToken._mint(deployer.address, expandDecimals(1000000, 18));
    await mintTx.wait();
